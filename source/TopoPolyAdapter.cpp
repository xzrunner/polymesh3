...
void TopoPolyAdapter::TransToPolymesh(std::vector<Polytope::PointPtr>& dst_pts,
                                      std::vector<Polytope::FacePtr>& dst_faces) const
{
    ...
    do {
        ...
        dst_pts.push_back(std::make_shared<Polytope::Point>(p3));
    ...
    {
        auto dst_f = std::make_shared<Polytope::Face>();
